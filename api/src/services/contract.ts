...
export const findERC20Token = async (address: string): Promise<ERC20Token> => {
  const res = await queryDb<GetERC20>(`SELECT contract_data as contractData, name, address FROM verified_contract WHERE type='ERC20' AND address=$1;`, [address]);
...
export const findTokenAccountTokenBalance = async (accountAddress: string, contractAddress: string): Promise<TokenBalace[]> => queryDb<TokenBalace>(
